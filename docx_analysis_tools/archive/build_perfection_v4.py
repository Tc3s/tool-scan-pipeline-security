...
# --- CHART 1: LAPTOP VULNERABILITY DISTRIBUTION CHART (FOR SECTION 2.1) ---
plt.figure(figsize=(7.5, 4.0))
categories_vi = ['NGHIÊM TRỌNG', 'CAO', 'TRUNG BÌNH', 'THẤP']
laptop_vals = [curr_laptop[c] for c in categories_vi]
colors_laptop = ['#FF0000', '#C00000', '#ED7D31', '#4472C4']

bars1 = plt.bar(categories_vi, laptop_vals, color=colors_laptop, width=0.45)
plt.xlabel('Mức độ nghiêm trọng (Severity)', fontweight='bold', fontsize=10)
plt.ylabel('Số lượng lỗ hổng', fontweight='bold', fontsize=10)
plt.title('BIỂU ĐỒ KẾT QUẢ DÒ QUÉT LỖ HỔNG MÁY TÍNH XÁCH TAY (192.168.95.135)', fontweight='bold', fontsize=11)
...
plt.savefig(chart_laptop_path, dpi=300)
plt.close()

# --- CHART 2: WEB SERVER VULNERABILITY DISTRIBUTION CHART (FOR SECTION 2.2) ---
plt.figure(figsize=(7.5, 4.0))
web_vals = [curr_webserver[c] for c in categories_vi]
colors_web = ['#FF0000', '#C00000', '#ED7D31', '#4472C4']

bars2 = plt.bar(categories_vi, web_vals, color=colors_web, width=0.45)
plt.xlabel('Mức độ nghiêm trọng (Severity)', fontweight='bold', fontsize=10)
plt.ylabel('Số lượng lỗ hổng', fontweight='bold', fontsize=10)
plt.title('BIỂU ĐỒ KẾT QUẢ DÒ QUÉT LỖ HỔNG WEB SERVER (192.168.95.138)', fontweight='bold', fontsize=11)
plt.grid(axis='y', linestyle='--', alpha=0.5)
...
plt.savefig(chart_web_path, dpi=300)
plt.close()


# --- CUSTOM VIETNAMESE DETAILS ---
VI_DETAILS = {
    # Laptop Findings
...
def get_custom_details_vi(finding_name, location, orig_desc, orig_ev, orig_sol):
    name_clean = clean_html(finding_name)
    if name_clean in VI_DETAILS:
        return VI_DETAILS[name_clean]
    
    loc = clean_html(location)
    d_clean = clean_html(orig_desc)
    return (
        d_clean[:250] if len(d_clean) > 10 else f"Phát hiện điểm yếu bảo mật thuộc dạng {name_clean} trên mục tiêu.",
...
add_p(f"Tổng số lỗ hổng phát hiện trên Máy tính xách tay Windows 11: {len(real_laptop)} lỗ hổng.")

# Insert Chart 1 here as requested
doc.add_picture(chart_laptop_path, width=Inches(6.0))
...
add_p(f"Tổng số lỗ hổng phát hiện trên Web Server: {len(real_webserver)} lỗ hổng.")

# Insert Chart 2 here as requested
doc.add_picture(chart_web_path, width=Inches(6.0))
doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
...
print("Successfully generated PERFECTION V4 Bao_Cao_An_Toan_Thong_Tin_2026.docx")
