...
```text
docx_analysis_tools/
├── README.md                                 # File tài liệu hướng dẫn kỹ thuật OpenXML
...
├── validate_report_perfection.py             # Bộ kiểm định tự động 16 tiêu chí chất lượng (16 Assertions)
├── full_vulnerability_knowledge_base.json    # Kho tri thức 35+ lỗ hổng đã được chuẩn hóa Tiếng Việt
├── archive/                                  # Thư mục lưu trữ lịch sử các builder thử nghiệm cũ (v2..v6)
└── debug_utils/                              # Thư mục chứa các script hỗ trợ inspect & gỡ lỗi OpenXML
```
